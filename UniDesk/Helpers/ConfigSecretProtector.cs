...
{
    private static byte[] DeriveKey(string appName = "UniDesk")
    {
        var seed = Encoding.UTF8.GetBytes(appName + "QWeather" + "Default" + "v1");
    ...
    public static string Decrypt(string cipherTextBase64)
    {
        ...
        if (TryDecryptWithKey("UniDesk", iv, cipher, out var plainText))
        {
            return plainText;
        }

        return TryDecryptWithKey("LumiDesk", iv, cipher, out plainText) ? plainText : string.Empty;
    }

    private static bool TryDecryptWithKey(string appName, byte[] iv, byte[] cipher, out string plainText)
    {
